...
export default {
  ...
  created() {
    ...
    axios.get('/articles/articleList').then(function(response) {
      if(response.status == 200) {
        if(response.data.code == 200) {
          that.getArticles = response.data.data; //监听函数，监听到变量改变即拿十条数据
          console.log("*****************");
          that.len = that.getArticles.length;
        } else {
    ...
    console.log(that.getArticles, "11111111111111111111111111111111111");
  ...
  methods: {
    ...
    showData() {
      console.log(this.getArticles, "222222222222222222");
    }
  },
  components: {
